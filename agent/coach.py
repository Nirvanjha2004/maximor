...
HARD CONSTRAINTS (violations are auto-rejected): POL-003 missing/invalid PO -> ESCALATE never REJECT; POL-005 unknown vendor -> ESCALATE never APPROVE/REJECT; POL-006 risk-flag -> ESCALATE never APPROVE; POL-007 closed PO -> ESCALATE never APPROVE; POL-002 tolerance breach (>2% or over $500 without open PO match) -> ESCALATE never REJECT; POL-004 duplicate window is 1-30 days strictly BEFORE the invoice date — same-day rows are the same record, recurring billing older than 30 days is legitimate, NEVER tell the agent to reject recurring invoices / approve regardless of duplicate flags / use any other window (no 12-month/extended lookbacks). Cite POL-00x rules only — never cite lesson IDs like L03 (they renumber)."""
